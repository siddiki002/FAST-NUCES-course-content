...
#include <string>
using namespace std;
bool check_right(char** board, string temp, int r, int c, int m, int n){        //r = i //c = j
    int check=0;
    for (int i = c,j=0; j < temp.size(), i<m; i++,j++)      // c = 3    i<temp.size()=4       // i 3 4 5 6 i max 4 
    {
        if (board[r][i]==temp[j] || board[r][i]=='+'){  //F I   
            check++;
        }
        else if (board[r][i]=='*'){
            char ch = temp[temp.size()-1];  //4 temp[3]
            while(i<m){     //i = c 
                if (ch==board[r][i]){       // E
                    return true;
                }
...
void find_word(char** board, string temp, int r, int c){        // temp = FIVE
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
        {
